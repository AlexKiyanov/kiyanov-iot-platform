...
class DeviceEventsListenerTest {
    ...
    void onBatch_WhenSingleEvent_ShouldProcessCorrectly() {
        // Given
        DeviceEvent event = createDeviceEvent("event-1", "device-1", 1000L, "TEMPERATURE", "25.5");
    ...
    void onBatch_WhenMultipleEvents_ShouldProcessAllCorrectly() {
        // Given
        DeviceEvent event1 = createDeviceEvent("event-1", "device-1", 1000L, "TEMPERATURE", "25.5");
        DeviceEvent event2 = createDeviceEvent("event-2", "device-1", 1001L, "HUMIDITY", "60.0");
        DeviceEvent event3 = createDeviceEvent("event-3", "device-2", 1002L, "TEMPERATURE", "26.0");
        ...
        assertThat(savedEntity2.getKey().getEventId()).isEqualTo("event-2");
        assertThat(savedEntity2.getKey().getDeviceId()).isEqualTo("device-1");
        
        // Verify third entity
        DeviceEventEntity savedEntity3 = savedEntities.get(2);
        assertThat(savedEntity3.getKey().getEventId()).isEqualTo("event-3");
    ...
    void onBatch_WhenMultipleEventsFromSameDeviceInLargeBatch_ShouldPublishDeviceIdOnlyOnce() {
        ...
        DeviceEvent event1 = createDeviceEvent("event-1", "device-1", 1000L, "TEMPERATURE", "25.5");
        DeviceEvent event2 = createDeviceEvent("event-2", "device-1", 1001L, "HUMIDITY", "60.0");
        DeviceEvent event3 = createDeviceEvent("event-3", "device-1", 1002L, "PRESSURE", "1013.25");
        DeviceEvent event4 = createDeviceEvent("event-4", "device-2", 1003L, "TEMPERATURE", "26.0");
        DeviceEvent event5 = createDeviceEvent("event-5", "device-2", 1004L, "HUMIDITY", "65.0");
    ...
    void onBatch_WhenEventsWithNullValues_ShouldProcessCorrectly() {
        // Given
        DeviceEvent event = createDeviceEvent("event-1", "device-1", 1000L, null, null);
        List<DeviceEvent> events = Collections.singletonList(event);
    ...
    void onBatch_WhenEventsWithEmptyStrings_ShouldProcessCorrectly() {
        // Given
        DeviceEvent event = createDeviceEvent("event-1", "device-1", 1000L, "", "");
        List<DeviceEvent> events = Collections.singletonList(event);
    ...
    void onBatch_WhenEventsWithZeroTimestamp_ShouldProcessCorrectly() {
        // Given
        DeviceEvent event = createDeviceEvent("event-1", "device-1", 0L, "TEMPERATURE", "25.5");
        List<DeviceEvent> events = Collections.singletonList(event);
